...
func SetupRoutes(h *handlers.Handler) *http.ServeMux {
        ...
        apimux.HandleFunc("DELETE /delete", h.User.DeleteUser)
        apimux.HandleFunc("PUT /users", h.User.UpdateProfile)
        apimux.HandleFunc("GET /notifications", h.User.GetNotification)
        apimux.HandleFunc("POST /follow", h.User.Follow)
        apimux.HandleFunc("POST /unfollow", h.User.Unfollow)
        apimux.HandleFunc("POST /avatar", h.User.UploadAvatar)
        apimux.HandleFunc("GET /avatar/{id}", h.User.GetAvatar)
